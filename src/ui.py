...
class UIElement:
    ...
    def __init__(self, center_pos, surface_normal, surface_hover=None, scale_on_hover=True):
        ...
        self.hover_image = surface_hover if surface_hover else surface_normal   # 若没有专门的 hover 图片，用原图
    ...
    def update(self, mouse_pos):
        ...
        if self.scale_on_hover:
            ...
            self.scale_factor += (self.target_scale - self.scale_factor) * 0.2  # 简单的线性插值动画 (Lerp)
...
class UpgradeCard(UIElement):
    ...
    def __init__(self, center_pos, size, bg_image, option_data, font_title, font_desc):
        ...
        # 缩放时，文字和图标会跟着背景一起缩放
        # 为了性能，将内容预渲染到 self.content_surface
        self.content_surface = self.base_surface.copy()
        cx = self.w // 2    # 计算布局坐标 (相对于卡片左上角)
        ...
        for char in text:
            test_line = current_line + char # 试探性加上这个字符，计算宽度
            w, h = font_desc.size(test_line)
            
            if w < max_width:
                current_line = test_line
            else:
                lines.append(current_line)  # 超出了，把之前的存入，当前字符作为新行开始
                current_line = char
        
        if current_line:
            lines.append(current_line)
        
        lines = lines[:3]   # 限制最多显示 3 行
        line_height = font_desc.get_height() + 5    # 绘制行
        start_y = self.h * 0.65 # 起始 Y 坐标
...
class UI:
    ...
    def __init__(self, display_surface, resource_manager):
        ...
        self.tutorial_mask.fill((0, 0, 0, 180))  
    ...
    def draw_hud(self, player):
        ...
        level_x = 20 + 300 + 30     # 血条右端位置：x=20, width=300，间距30
        # 垂直居中，与血条对齐
    ...
    def setup_level_up(self, options):
        ...
        for i, opt in enumerate(options):
            ...
            if hasattr(opt, 'icon_key') and opt.icon_key:
                ...
                card.hover_image = final_surf # 悬停时缩放
    ...
    def draw_main_menu(self):
        """绘制主菜单"""
        self.display_surface.blit(self.menu_bg, (0, 0)) # 绘制背景图片
