...
class Game:
    ...
    def draw(self):
        if self.state == 'MENU':
            ...
            # 其他状态：绘制游戏内容
            self.screen.fill(COLORS['bg_void'])     # 地图外使用黑色填充
    ...
    def events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.TEXTINPUT:
                # 防止中文输入法在按下 WASD 时显示输入框
                pass
            elif event.type == pygame.TEXTEDITING:
                # 文本编辑事件（输入法候选词等），忽略
                pass
            
            if event.type == pygame.KEYDOWN:    # 防止WASD输入被输入法拦截
