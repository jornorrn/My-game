...
```mermaid
graph TD
    Camera["YSortCameraGroup相机组"] --> Ground["ground层-地板装饰"]
    Camera --> VFXBottom["vfx_bottom层-阴影地面特效"]
    Camera --> Main["main层-玩家敌人障碍物"]
    Camera --> VFXTop["vfx_top层-爆炸伤害特效"]
...
#### 问题描述
使用WASD键控制角色移动时，系统输入法会拦截按键，导致游戏无法正常接收输入。

#### 问题分析
中文输入法会拦截键盘事件，特别是WASD等常用键，导致pygame无法接收到按键事件。
...
**开发环境**：Python 3.13.3, Pygame-CE 2.5.6, Windows 11